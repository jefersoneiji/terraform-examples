  ...
  lifecycle {
    ...
    postcondition {
      ...
      error_message = <<-EOT
        ...
        Please ensure that the subnet is properly tagged by adding the following tag:
        1. Access = "private"
      EOT
    }
  }
  ...
  lifecycle {
    postcondition {
      ...
      error_message = <<-EOT
        ...
        1. Rules must noa allow inbound traffic from IP CIDR blocks, only from other security groups.
      EOT
    }
  }
}
